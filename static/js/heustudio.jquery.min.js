...
$(document).ready(function(){
  ...
  if($('#investModal').length	> 0){
  	...
  	if($('#invest-money-gu').length > 0){
  		$('#invest-money-gu').keyup(function(){
  			var per_price = parseFloat($('#st_per_price_text').val());
  			var input = $('#invest-money-gu').val();
  			var reg = /^\d+(\.\d+)?$/;
  			if(!reg.test(input)){
  				$('#invest-money').text('0');
  			}else{
  				$('#invest-money').text(parseFloat(parseFloat(input)*per_price).toFixed(2));
  			}
  		});
  	}
